...
{#snippet segment(index: number, position: string, vertical: boolean, dot: boolean = false)}
  <div
    ...
    style="height: {vertical && !dot ? segLength : segWidth}rem; width: {vertical || dot
      ? segWidth
      : segLength * aspectRatio}rem; {position}; scale: {vertical || dot ? 1 : (segLength + segWidth / 1) / segLength} 1;">
  </div>
{/snippet}

<div
  class="relative"
  style="width: {(segLength * aspectRatio + 2 * segWidth) * (1 + padding)}rem; height: {(segLength * 2 + 3 * segWidth) *
    (1 + padding)}rem">
  {#if pattern}
    <!-- Segment A -->
    ...
    <!-- Segment C -->
    {@render segment(2, `bottom: ${segWidth * (1 + (2 * padding) / aspectRatio)}rem; right: 0;`, true)}

    <!-- Segment D -->
    {@render segment(3, `bottom: 0; left: ${segWidth * (1 + 2 * padding)}rem;`, false)}

    <!-- Segment E -->
    {@render segment(4, `bottom: ${segWidth * (1 + (2 * padding) / aspectRatio)}rem; left: 0;`, true)}

    <!-- Segment F -->
    {@render segment(5, `left: 0; top: ${segWidth * (1 + (2 * padding) / aspectRatio)}rem;`, true)}

    <!-- Segment G -->
    {@render segment(6, `left: ${segWidth * (1 + 2 * padding)}rem; top: 50%; margin-top: -${segWidth / 2}rem;`, false)}
