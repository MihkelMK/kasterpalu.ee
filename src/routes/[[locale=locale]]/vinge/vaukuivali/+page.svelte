<script lang="ts">
  ...
  function getCurrentCheckpoint(arr: number[], current: number) {
    return (
      arr.reduce((prev: number | undefined, item) => (item <= current && (!prev || item > prev) ? item : prev), undefined) ||
      arr.at(0)
    );
  }
...
{#snippet checkpoint(db: number, passed: boolean)}
  <div
    class="rounded-r-lg transition-colors {passed
      ...
      : 'bg-muted-foreground text-muted'} py-1 text-center {displayDecimals ? 'w-16 text-sm' : 'w-14 text-xs'}">
    {db}dBA
  </div>
{/snippet}
...
<svelte:window onkeydowncapture={disableHomeAndEnd} bind:scrollY={scrollY.target} bind:innerWidth bind:innerHeight />

{#if innerWidth === 0 || innerHeight === 0}
  <div class="grid h-screen w-full items-center justify-center" out:fade={{ duration: 150, easing: expoOut }}>
    <LoaderCircle class="animate-spin" />
  </div>
{:else}
  <div style="height: {containerHeight}px;" class="relative mb-24 w-full" in:fade={{ duration: 300, easing: expoOut }}>
    <div class="sticky top-24 left-4 flex items-start rounded-xl">
        ...
        <div class="flex flex-col items-center gap-4">
          <div
            ...
            {#each checkpointDecibels as db ('checkpoint-' + db)}
              <div transition:fade style="top: calc({(db / 150) * 70}svh - 0.5rem)" class="absolute flex items-center">
      ...
      <div
        ...
        {#if typeof currentCheckpoint === 'number'}
          <Image image={soundCheckpoints[currentCheckpoint]?.image} class="aspect-square object-cover" />
          <header
    ...
    <div>
      ...
      <ul class="my-4 ml-6 list-disc [&>li]:mt-2">
        <li class="max-w-prose text-sm leading-4 text-muted-foreground">
          ...
          doi:10.1007/978-1-4419-6712-1_8
        </li>
        <li class="max-w-prose text-sm leading-4 text-muted-foreground">
          <a
